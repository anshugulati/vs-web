...
export class BlockRenderer extends LitElement {
  ...
  handleMutations(mutations) {
    mutations.forEach((mutation) => {
      ...
      // item without library-id could be modified programmatically and
      // hence modifiedElementHTML might be null
      if (!modifiedElementHTML) return;
      ...
      // Was an image DND'd?
        ...
        if (mutation.attributeName === 'href'
          && target.tagName === 'A') {
          modifiedElementHTML.href = target.href;
          modifiedElementHTML.innerText = target.href.split('/')?.pop()?.split('?')[0];
        }
