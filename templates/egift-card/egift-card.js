...
import {
  closeModal, createModal, fetchPlaceholdersForLocale, formatPrice,
  getBrandPath, getLanguageAttr, openModal,
...
async function showBalance(modalContent, data, placeholders) {
  ...
  modalContent.innerHTML = `
    ...
    <p class="valid-upto">${placeholders.egiftBalanceModalCardValid || 'Card valid upto'} ${data.expiry_date}</p>
  `;
...
async function openBalanceModal(placeholders) {
  ...
  checkBalanceBtn.setAttribute('type', 'button');
  checkBalanceBtn.addEventListener('click', async (e) => {
    ...
    if (!validateInput(cardNumberField)) {
      target.disabled = false;
      return;
    }
    if (!checkBalanceForm.classList.contains('no-otp') && otpField.value.trim() === '') {
      showErrorMessage(otpField, invalidOtpError);
      otpField.focus();
      target.disabled = false;
      return;
    }
...
export default async function decorate(main) {
  ...
  const checkBalanceLink = document.querySelector('a[href$="/egift-card/balance"]');
