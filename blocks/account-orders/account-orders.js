...
import { getOrderProductNames, formatDateTime } from '../account-recent-orders/account-recent-orders.js';
import {
  getOrderStatus, ORDER_STATUSES, getTotalItemsCount, getCancelledOrderCount,
} from '../account-recent-orders/account-recent-orders-util.js';

const PAGE_SIZE = 10;
const FILTER_OPTIONS = ['ALL ORDERS', 'CANCELLED', 'COLLECTED', 'COMPLETE', 'DELIVERED', 'DISPATCHED', 'PROCESSING', 'READY_TO_COLLECT', 'REFUNDED', 'RETURNED'];
  ...
  orders.items.forEach(async (order) => {
    ...
    if (cancelledItems > 0) {
      ...
      orderItemsCancelled.addEventListener('click', (event) => {
        ...
        window.location.href = `${viewDetailsLink}${order.increment_id}#cancelled-items-section`;
        setTimeout(() => {
    ...
    orderExpandLink.href = `${viewDetailsLink}${order.increment_id}`;
    ...
    orderNumber.addEventListener('click', () => {
      window.location.href = `${viewDetailsLink}${order.increment_id}`;
    });
    orderStatus.addEventListener('click', () => {
      window.location.href = `${viewDetailsLink}${order.increment_id}`;
    });
    orderTotal.addEventListener('click', () => {
      window.location.href = `${viewDetailsLink}${order.increment_id}`;
    });
  });
