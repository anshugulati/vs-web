...
import {
  getBreadcumbCategories, getBreadcumbCategoriesForProduct, isPDP, isPLP, setJsonLd,
...
export async function buildBreadcrumbs() {
  ...
  if ((template === 'department' || isPLP()) && !pathName.includes('/wishlist') && !pathName.includes('/my-favourites')) {
    const urlKey = buildUrlKey();
    const cleanedUrlKey = urlKey.includes('/view-all')
      ? urlKey.replace(/\/view-all$/, '')
      : urlKey;
    breadcrumbCategories = await getBreadcumbCategories(cleanedUrlKey);
    ...
    if (breadcrumbCategories?.commerce_categories) {
      ...
      if (!document.querySelector('.sidebar-wrapper .sidebar .sidebar-item')) {
        ...
        sendCategoriesToDataLayer(categoryListName, null, null, false, true);
      }
    }
  } else if (isPDP()) {
  ...
  ol.append(...crumbs.map((item, idx) => {
    ...
    if (item.url) {
      ...
      li.append(a);
      itemMeta['@id'] = a.href;
      itemMeta.name = a.title;
    } else {
      li.append(span);
      itemMeta.name = item.title;
      if (idx === crumbs.length - 1) {
        itemMeta['@id'] = window.location.href;
      }
    }
    entries.push({
      '@type': 'ListItem',
