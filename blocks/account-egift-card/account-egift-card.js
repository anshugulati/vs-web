...
async function decorateReloadBlock(form, linkedGiftCards, placeholders) {
  const minPrice = extractNumber(placeholders.minMonetaryValue);
  const maxPrice = extractNumber(placeholders.maxMonetaryValue);
...
`).join('');
  ...
  reloadBlock.innerHTML = `
      <div class="egift-card-reload-amount-wrapper">
          ...
          <div class="egift-card-reload-amount">
            ...
            <div class="input-field-wrapper">
              <div class="input-wrapper input-field">
                <input type="number" class="egift-reload-amount" id="egiftReloadAmount" placeholder=" " min=${minPrice} max=${maxPrice} step="1">
          ...
          <div class="egift-card-reload-button-container">
            ...
            <button type="submit" class="egift-card-reload-button" id="egiftCardReloadButton" disabled>${placeholders.egiftTopUp || 'Top up'}</button>
          </div>
  ...
  warningMessage.textContent = placeholders.egiftErrorMsgAmountTopUp.replace('{{minPrice}}', placeholders.minMonetaryValue).replace('{{maxPrice}}', placeholders.maxMonetaryValue) || 'Please enter amount in range of {{minPrice}} and {{maxPrice}}';
...
async function decorateUnlinkedForm(form, placeholders) {
  ...
  form.innerHTML = `
      ...
      <button type="submit" class="get-code" id="getCode">${placeholders.egiftGetcode || 'Get code'}</button>
...
async function unlinkModal(form, placeholders) {
  ...
  unlinkModalContent.innerHTML = `
    <span class="unlink-card-model-title">${placeholders.unlinkModalHeading || 'Unlink Card'}</span>
  ...
  unlinkModalContainer.insertBefore(buttonNo, buttonYes);
...
<div class="egift-card-info-bottom-container">
  ...
  <button type="submit" class="egift-card-reload" id="egiftCardReload">${placeholders.egiftTopUp || 'Top Up'}</button>
</div> 
    `;
