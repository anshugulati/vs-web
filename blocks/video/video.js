...
function parseConfig(block) {
  ...
  const captions = [
    {
      label: 'English',
      srclang: 'en',
      src: 'path/to/english-captions.vtt', // there is no support for captions in DAM for Alshaya ATM, that's why these are placeholders
      default: true,
    },
    {
      label: 'Arabic',
      srclang: 'ar',
      src: 'path/to/arabic-captions.vtt',
    },
  ];
  ...
  if (block.classList.contains('embed')) {
    ...
    const title = block.querySelector('h1, h2, h3')?.textContent;
    const description = block.querySelector('div:nth-child(2) > p')?.textContent;
...
function setupPlayer(url, videoContainer, config) {
  ...
  if (config.captions && Array.isArray(config.captions)) {
  ...
  // for some reason passing posterImage=null to video.js causes a crash so we explicitly delete it
  if (!videojsConfig.posterImage) {
    delete videojsConfig.posterImage;
  }
  console.warn('Poster image not found, removing posterImage property.');

  // eslint-disable-next-line no-undef
  const player = videojs(videoElement, videojsConfig);
  player.src(url);

  // Adjust the container dimensions based on the video's aspect ratio
...
async function decorateEmbedBlock(block, config) {
  const container = document.createElement('div');
  container.classList.add('video-inline');
...
export default async function decorate(block) {
  ...
  if (config.type === 'cards') {
    ...
    videoLink.classList.add('video-bg-link');
    const videoWrapper = videoLink ? `<span class="video-bg-lbl">VideoLink :</span> ${videoLink?.outerHTML}` : '';
    await decorateVideoCards(block, config);
    if (isSidekickLibrary) {
      const videoDiv = document.createElement('div');
      videoDiv.classList.add('sidekickVideo');
      videoDiv.innerHTML = videoWrapper;
  ...
  if (config.type === 'embed') {
    await decorateEmbedBlock(block, config);
    return;
  }

  if (config.videoUrl?.includes('youtube.com') || config.videoUrl?.includes('youtu.be')) {
    const { loadEmbed } = await import('../embed/embed.js');
    loadCSS(`/blocks/embed/${getBrandPath()}embed.css`);
    loadEmbed(block, config.videoUrl);
    return;
  }
