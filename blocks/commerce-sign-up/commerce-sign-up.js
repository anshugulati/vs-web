...
import { datalayerLogin, getPageName } from '../../scripts/analytics/google-data-layer.js';
...
async function signup(form, placeholders, lang, countryIso) {
  ...
  const customerGender = form.querySelector('.gender .select-selected').dataset
    .value;
  const phoneNumber = await phoneWithIso(countryIso);

  const response = await fetch(
    `${commerceRestEndpoint}/${storeCode}/V1/customers`,
    {
      method: 'POST',
      headers: {
  ...
  if (response.ok && data?.email === email) {
    ...
    successContainer.textContent = placeholders.signupSuccessMessage
      || `Account created successfully for ${data.firstname}. Redirecting to login page...`;
    successContainer.textContent = successContainer.textContent?.replace(
      '{{}}',
      data.firstname,
    );
    ...
    document
      .querySelector('.submit-container button.createNewAccountButton')
    ...
    if (errorMessage.includes('same email address')) {
      ...
      showErrorMessage(
        ...
        placeholders.alreadyTakenMail || 'The email address is already taken',
      );
    } else {
...
<div class="success-container"></div>
  ...
  const getMoreContainer = form.querySelector('.commerce-sign-up.accordion.plus');
  ...
  document
    .querySelector('.unmask-password')
    .addEventListener('click', togglePasswordVisibility);
  form.addEventListener('submit', async (event) => {
    event.preventDefault();
    const submitButton = document.querySelector(
      '.submit-container button.createNewAccountButton',
    );
    ...
    grecaptcha.ready(async () => {
      ...
      if (!token) {
        showPageErrorMessage(
          placeholders.recaptchaErrorMessage
      ...
      if (captchaValidated) {
        ...
        submitButton?.classList.remove('loader');
        const recaptchaErrorMessage = placeholders.recaptchaErrorMessage
          || 'Verification failed. Please try again.';
