...
function toggleSections(action) {
  if (action === 'hide') {
    ...
    document.querySelector('.egift-action-btns').classList.add('details-form');
    if (!document.querySelector('.custom-amount.selected')) {
      document.querySelector('.egift-content-right .input-field-wrapper').classList.remove('egift-visible');
      document.querySelector('.egift-content-right .egift-custom-amount').classList.remove('egift-visible');
    }
  } else {
    ...
    document.querySelector('.egift-content-right .egift-custom-amount').classList.add('egift-visible');
    document.querySelector('.egift-action-btns').classList.add('details-form');
  }
}
...
function decorateEgiftPurchase(block) {
  ...
  block.querySelector('h3').classList.add('egift-custom-amount');
  ...
  contentRight.querySelector('.input-field-wrapper').classList.add('egift-custom-amount-input');
...
function updatePrimaryImage() {
  document.querySelector('.egift-image').src = window.egiftItems[egiftIndex].image;
  document.querySelector('.egift-image').setAttribute('title', window.egiftItems[egiftIndex].name);
  document.querySelector('.egift-image').setAttribute('alt', window.egiftItems[egiftIndex].name);
}
...
function updatePriceOptions() {
  ...
  window.egiftItems[egiftIndex].priceOptions.forEach((price) => {
...
function updateContent() {
  updatePrimaryOptions(window.egiftItems[egiftIndex]);
  window.egiftItems.forEach((item) => {
  ...
  document.querySelector('p.egift-card-name').innerText = window.egiftItems[egiftIndex].name;
...
function updateInputValidation() {
  document.querySelectorAll('.input-field-wrapper').forEach((ele) => {
    ...
    } else if (ele.querySelector('textarea')) {
...
function validateFields() {
  ...
  document.querySelectorAll('.input-field-wrapper input').forEach((input) => {
    if (input.type === 'email') {
      ...
      const validInput = parseFloat(input.value)
        >= parseFloat(window.egiftItems[egiftIndex].minPrice)
        && parseFloat(input.value) <= parseFloat(window.egiftItems[egiftIndex].maxPrice);
      if (input.value === '' || !validInput) {
        input.parentElement.parentElement.querySelector('.error-message-container .error-message').innerText = amountError.replace('{{minPrice}}', placeholders.minMonetaryValue).replace('{{maxPrice}}', placeholders.maxMonetaryValue);
        input.parentElement.parentElement.querySelector('.error-message-container').classList.remove('hide');
        input.classList.add('invalid');
        isValid = false;
        toggleSections('hide');
...
function validateInputFields(input) {
  if (input.type === 'email') {
    ...
    const validInput = parseFloat(input.value) >= parseFloat(window.egiftItems[egiftIndex].minPrice)
      && parseFloat(input.value) <= parseFloat(window.egiftItems[egiftIndex].maxPrice);
    if (input.value === '' || !validInput) {
      input.parentElement.parentElement.querySelector('.error-message-container .error-message').innerText = amountError.replace('{{minPrice}}', placeholders.minMonetaryValue).replace('{{maxPrice}}', placeholders.maxMonetaryValue);
      input.parentElement.parentElement.querySelector('.error-message-container').classList.remove('hide');
...
async function attachEvents() {
  ...
  document.querySelector('.egift-add-to-bag').addEventListener('click', async (e) => {
    e.preventDefault();
    if (e.target.closest('.button-container').classList.contains('disabled-btn')) {
      return;
    }
    if (validateFields()) {
      ...
      const options = {
        product_type: window.egiftItems[egiftIndex].productType,
      ...
      await cartApi.addToCart(window.egiftItems[egiftIndex].sku, options, 1, 'egift-purchase');
...
export default async function decorate(block) {
  ...
  amountError = placeholders.egiftErrorMsgAmountTopUp || 'Please enter amount in range of {{minPrice}} and {{maxPrice}}';
  decorateEgiftPurchase(block);

  loadConfigs().then(() => {
    if (window.egiftItems.length > 0) {
      updateContent();
      attachEvents();
    }
  });
