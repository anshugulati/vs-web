import {
  isLoggedInUser, fetchPlaceholdersForLocale, formatDateToCurrentLocale, loadFragment,
...
async function decorateAccordion(parent, placeholders, pointsEarned, fragmentPath, accordionText) {
  ...
  if (fragmentPath) {
    loadFragment(fragmentPath).then((accordionFragment) => {
      const accordionTextContent = accordionFragment?.outerHTML || '';
      accordionFooter.innerHTML = accordionTextContent;
    });
  } else if (accordionText) {
    accordionFooter.innerHTML = accordionText;
  }
...
export default async function decorate(block) {
  ...
  const fragmentPathLink = block.querySelector('a');
  const fragmentPath = fragmentPathLink?.getAttribute('href') || '';
  const accordionContent = block.innerHTML;
