import {
  fetchPlaceholdersForLocale, createModal, openModal, closeModal, loadFragment,
...
export default async function decorate(block) {
  ...
  if (!tierProgressData || !tierProgressData.extension_attributes) {
    block.innerHTML = '';
    return;
  }
  ...
  const isNewHelloMember = +(memberData?.extension_attributes?.is_new_hello_member ?? 0) === 1;
  const isPlusMember = memberData.apc_points >= Math.max(...predefinedPoints);

  if (isNewHelloMember) {
    const lang = document.documentElement.lang || 'en';
    const helloMemberFragment = await loadFragment(`/${lang}/fragments/banners/offers/hello-member-welcome-banner`);
    await createModal('hello-member-welcome-modal', '', helloMemberFragment.outerHTML, ['hello-member-welcome-modal']);
    openModal('hello-member-welcome-modal');
    const closeBtn = document.querySelector('.hello-member-welcome-modal .banner-content .button');
    closeBtn.addEventListener('click', (e) => {
      e.preventDefault();
      closeModal('hello-member-welcome-modal');
    });
  }
  ...
  if (isPlusMember) {
    ...
    if (isRtl) {
      startCircle.style.right = '0';
      endCircle.style.left = '0';
    } else {
