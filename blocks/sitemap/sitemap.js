      ...
      for (let i = 0; i < pathParts.length; i += 1) {
        ...
        if (!childNode) {
          childNode = {
            ...
            include_in_menu: item['hide-from-sitemap'] === 'true' ? 0 : 1,
            id: `static-${idx}`,
...
const generateHTML = (categories, viewAllPlaceholder, level = 1) => {
  ...
  categories.forEach((node) => {
    ...
    let hrefPath = node.url_path;

    // Ensure the path starts with `/${lang}/`
    if (!hrefPath.startsWith(`${lang}/`)) {
      hrefPath = hrefPath === lang ? `/${lang}` : `/${lang}/${hrefPath}`;
    } else {
      hrefPath = `/${hrefPath}`;
    }
    ...
    if (node?.children?.length > 0) {
      const nestedUl = generateHTML(node.children, viewAllPlaceholder, null);
