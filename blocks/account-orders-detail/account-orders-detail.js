...
} from '../account-recent-orders/account-recent-orders-util.js';
import {
  decorateReviews,
...
const getDownloadInvoiceBtn = async (order, placeholders) => {
  ...
  try {
    ...
    const downloadLink = await createUrl(`order-manager/customers/invoice/${order.increment_id}`);
    ...
    invoiceBtn.setAttribute('role', 'button');
    invoiceBtn.setAttribute('download', '');
    invoiceBtn.innerText = placeholders?.orderDownloadInvoice || 'Download Invoice';

    invoiceBtn.addEventListener('click', async (event) => {
      event.preventDefault();
      const authToken = getSignInToken();
      // adding timeout to cancel the request if it exceeds the specified time
      const timeout = 10000;
      try {
        ...
        const timeFormat = new Date().toISOString().replace(/[^0-9]/g, '').slice(0, 12);
        const pdfUrl = URL.createObjectURL(blob);
        // since content-disposition headers are not present,
        // we create a temp a tag and assign filename to it
        const link = document.createElement('a');
        link.href = pdfUrl;
        link.download = `Invoice_${timeFormat}.pdf`;
        ...
        if (error.name === 'AbortError') {
          ...
          console.error('Error occurred while trying to download invoice: ', error);
        }
      }
    });
    return invoiceBtn;
...
export function getDeliveredItemsCount(item) {
  ...
  return item.qty_ordered - item.qty_refunded || 0;
}

export async function renderUnitInfo(placeholders, order, item, isDeliveredList) {
  ...
  const [originalPrice, finalPrice] = await Promise.all([
    formatPrice(order.order_currency_code, item.original_price),
    item.price_incl_tax === FREE_ITEM_PRICE
      ? placeholders.orderPriceFree ?? 'Free'
      : formatPrice(order.order_currency_code, item.price_incl_tax),
  ]);

  if (!isDeliveredList && originalPrice && finalPrice
  && (item.original_price > item.price_incl_tax)) {
    const itemOriginal = document.createElement('h6');
    itemOriginal.classList.add('item-price-original-slashed', 'text-dark');
    itemOriginal.textContent = originalPrice;

    const itemDiscounted = document.createElement('p');
    itemDiscounted.classList.add('item-price-discounted');
    itemDiscounted.textContent = finalPrice;

    const discountPercentage = document.createElement('p');
    discountPercentage.classList.add('item-price-discount-value');
    discountPercentage.textContent = `(${placeholders.plpSave}
    ${getDiscountPercentage(item.original_price, item.price_incl_tax)}%)`;

    unitInfo.appendChild(itemOriginal);
    unitInfo.appendChild(itemDiscounted);
    unitInfo.appendChild(discountPercentage);
  } else {
    unitInfoPrice.textContent = finalPrice;
  }
  return { unitInfo, unitInfoPrice };
}
...
const buildOrderList = async (
  ...
  items.forEach(async (item) => {
    ...
    const orderSwatchImg = await getProductImage(product);
    if (isCancelled) {
    ...
    // Quantity of an item
    let itemQuantity = isCancelled ? getCancelledItemsCount(item, cancelDelivery)
      : getDeliveredItemsCount(item);
    ...
    // Unit price details
    const { unitInfo, unitInfoPrice } = await renderUnitInfo(
      placeholders,
      order,
      item,
      isDeliveredList,
    ...
    let qcStatus;
    if (qcFailStatus) {
      qcStatus = document.createElement('div');
      qcStatus.classList.add('qc-info');
      const reasons = item?.reason_description;
      if (Array.isArray(reasons)) {
        reasons.forEach((reason) => {
          const qcValue = document.createElement('p');
          qcValue.classList.add('list-item-head-status-label', 'goods-collection-failed');
          qcValue.textContent = reason || '';
          qcStatus.appendChild(qcValue);
        });
      }
    }
  ...
  rmaOrder.items.forEach((rmaItem) => {
    order.items.forEach((orderItem) => {
      if (orderItem.item_id === rmaItem.order_item_id) {
        ...
        const reasonDescriptions = (rmaItem.extension_attributes?.reason_description || []).map(
          (desc) => desc.reason_description,
        );
        // Attach reason_description to orderItem if available
        if (reasonDescriptions.length > 0) {
          orderItem.reason_description = reasonDescriptions;
        }
        inOrderItems.push(orderItem);
      }
    });
  });
  const inOrderItemsClone = JSON.parse(JSON.stringify(inOrderItems));
  // Creates the return items list
  const rmaBucketItemList = await buildOrderList(
    order,
    inOrderItemsClone,
    productData,
    placeholders,
...
const renderReturnRmaBucket = async (rmaData, order, productData, placeholders, locale) => {
  ...
  processedRmaData.forEach(async (rmaOrder) => {
    const isCancelDisabled = ['goods_collected', 'refunded', 'return_rejected'].includes((rmaOrder)?.extension_attributes?.customer_status_key);
    ...
    returnRmaBucketHeadMsg.innerText = rmaOrder?.extension_attributes?.description ? ` - ${(rmaOrder?.extension_attributes?.description)}` : '';
...
const renderCancelRmaBucket = async (rmaData, order, productData, placeholders, locale) => {
  if (rmaData?.total_count === 0) {
    return null;
  }

  const processedRmaData = Array.isArray(rmaData.items)
    ? rmaData.items
      .map((rmaOrder) => {
        const itemsWithReason = Array.isArray(rmaOrder.items)
          ? rmaOrder.items
            .filter((item) => item.extension_attributes?.reason_description != null)
            .map((item) => ({
              ...item,
              reason_description: item.extension_attributes.reason_description,
            }))
          : [];
        return itemsWithReason.length > 0 ? { ...rmaOrder, items: itemsWithReason } : null;
      })
      .filter(Boolean)
    : [];
  ...
  processedRmaData.forEach(async (rmaOrder) => {
    ...
    title.classList.add('cancelled-itemized-title');
    title.innerHTML = `
      <h5>${placeholders?.onlineReturnCancel || 'CANCELLED ITEM - ONLINE RETURNS'}</h5>
    `;
    returnRmaBucketHeadStatus.appendChild(title);
    returnRmaBucketHeadLeft.appendChild(returnRmaBucketHeadStatus);
 ...
 */
const getOrderPaymentMethodName = (order, placeholders) => {
  const paymentMethod = order?.payment?.method;
  const formattedPaymentMethod = paymentMethod?.replace(/_([a-z])/g, (match, letter) => letter.toUpperCase());
  let paymentMethodTitle = placeholders[formattedPaymentMethod]
  || order?.extension_attributes?.payment_additional_info?.find((info) => info.key === 'method_title')?.value;

  const hpsRedemption = order?.extension_attributes?.payment_additional_info?.find((info) => info.key === 'hps_redemption');
  const hpsRedemptionTitle = placeholders?.orderHpsRedemption || 'eGift Card';
  if (hpsRedemption) {
    if (!paymentMethodTitle) {
      paymentMethodTitle = hpsRedemptionTitle;
    } else {
      paymentMethodTitle += `, ${hpsRedemptionTitle}`;
    }
  }
...
const buildHomeDeliveryDetails = async (order, placeholders) => {
  ...
  const paymentMethod = getOrderPaymentMethodName(order, placeholders);
  ...
  if (order?.payment?.method === 'checkout_com_upapi_knet') {
    ...
    if (paymentId) {
      paymentDetails += `<h6 class="text-dark">${placeholders?.orderPaymentId || 'Payment ID'}</h6>
        <p class="break-word">${paymentId}</p>`;
    }
  ...
  deliveryDetailsContent.innerHTML = `
    <div class="delivery-details-info">
      ...
      <div class="address">
        ...
        <p>${delCity}</p>
        <p>${delArea}</p>
        <p>${await sanitizeDOM(delStreet)}</p>
        <p>${await sanitizeDOM(delBuilding)}</p>
      </div>
    ...
    <div class="billing-details-info">
      ...
      <div class="address">
        ...
        <p>${billCity}</p>
        <p>${billArea}</p>
        <p>${await sanitizeDOM(billStreet)}</p>
        <p>${await sanitizeDOM(billBuilding)}</p>
      </div>
...
const buildClickNCollectDeliveryDetails = async (order, placeholders) => {
  ...
  const storeCode = order?.extension_attributes
    ?.shipping_assignments?.[0]
    ?.shipping?.extension_attributes
    ?.store_code;
  ...
  const orderStore = storeData.items.find((store) => store.store_code === storeCode);
  ...
  const collectorPhone = order.extension_attributes
    ?.shipping_assignments[0]
    ?.shipping
    ?.address
    ?.telephone;
  ...
  collectionAddressInfo.innerHTML = `
    ...
    <div class="address">
      ...
      <p>${collectArea}</p>
      <p>${collectCity}</p>
      <p class='mobile-number-order-details'>${collectStorePhone}</p>
    </div>
    ...
    <div class="mobile">
      ...
      <p class='mobile-number-order-details'>${countryPrefix}${collectorPhone}</p>
    </div>
  `;
...
const renderDeliveredItems = async (order, productData, placeholders, locale) => {
  ...
  const totalDeliveredQuantity = deliveredItems
    .reduce((acc, item) => acc + item.qty_ordered - item.qty_refunded, 0);
  ...
  const deliveredList = await buildOrderList(
    order,
    deliveredItems,
    productData,
    placeholders,
 ...
 * @returns
 */
const renderCancelledItems = async (order, productData, placeholders, locale, rmaData) => {
  const isCancelled = true;
  const cancelDelivery = order.status === 'cancelled_stock_short' || order.status === 'cancelled_failed_delivery';
  const cancelledItems = getOrderCancelledItems(order, rmaData, cancelDelivery);
  ...
  const cancelledList = await buildOrderList(
    order,
    cancelledItems,
    productData,
    placeholders,
...
const renderOrderTotals = async (order, placeholders) => {
  ...
  if (order.discount_amount !== 0) {
    const absoluteDiscountValue = Math.abs(order.discount_amount);
    const discountFormatValue = await formatPrice(order.order_currency_code, absoluteDiscountValue);
    const lang = document.documentElement?.lang || 'en';
    ...
    if (lang === 'ar') {
      [amount, currencySymbol] = discountFormatValue.split(/\u00A0/);
      dicountedText = `${amount}- ${currencySymbol}`;
    } else {
      [currencySymbol, amount] = discountFormatValue.split(/\u00A0/);
      dicountedText = `${currencySymbol} -${amount}`;
    }
    ...
    surcharge.innerHTML = `
      ...
      <div class="price">
        ...
        <div class="surcharge-price-info">
          <img src="/icons/info-white.svg" id="surcharge-price-info-icon" />
          <span class="hidden">
 ...
 * @param {*} locale The locale string
 */
const renderOrderDetails = async (block, incrementId, placeholders, locale) => {
  const orders = await searchOrderWithIncrementId(incrementId);
  ...
  const orderId = orders.items[0]?.entity_id;
  ...
  renderHeaderButtons(order, placeholders);
  ...
  // for cancel items---------------------------------------
  const returnCancelRmaBucket = await renderCancelRmaBucket(
    rmaData,
    order,
    productData,
    placeholders,
  ...
  if (returnCancelRmaBucket) {
    block.appendChild(returnCancelRmaBucket);
  }

  // Renders the list of items that are cancelled as part of the order
  const cancelledItems = await renderCancelledItems(
    order,
    productData,
    placeholders,
    locale,
