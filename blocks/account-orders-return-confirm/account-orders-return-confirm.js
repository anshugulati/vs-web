...
import { getOrder, getRMAInfo, searchOrderWithIncrementId } from '../../scripts/order/api.js';
import { fetchPlaceholdersForLocale, getLocale, sanitizeDOM } from '../../scripts/scripts.js';
...
import { fetchProductDataForOrders } from '../account-recent-orders/account-recent-orders-util.js';
import { formatDateTime } from '../account-recent-orders/account-recent-orders.js';
...
const renderRefundMethodInfoBox = (orderData, returnData, placeholders) => {
  ...
  switch (refundMethod) {
    ...
    case 'checkout_com_upapi':
      if (orderData?.extension_attributes?.payment_additional_info) {
        const type = orderData?.extension_attributes?.payment_additional_info?.find((info) => info.key === 'scheme')?.value;
        icon = type?.toLowerCase();
        const maskedCC = orderData?.extension_attributes?.payment_additional_info?.find((info) => info.key === 'last4')?.value || '';
 ...
 * @param {String} locale The locale string
 */
const renderOrderConfirmation = async (block, incrementId, returnId, placeholders, locale) => {
  if (!(incrementId && returnId)) {
    return;
  }
  ...
  renderHeaderButtons(orderData, placeholders);
