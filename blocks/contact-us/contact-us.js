import { fetchPlaceholdersForLocale, sanitizeDOM } from '../../scripts/scripts.js';
import {
...
async function validatePhoneInput(phoneInput, placeholders) {
  phoneInput.value = phoneInput.value.replace(/[^0-9]/g, '');
  const isValidPhoneNumber = await validatePhone(
    phoneInput.value,
    phoneInput.dataset.countryCode || 'AE',
  );
...
export default async function decorate(block) {
  if (block.classList.contains('radio-selection')) {
    import('./contact-us-radio.js').then((module) => {
    ...
    <label for="communication-channel">${placeholders.contactUsCommunicationChannel || 'Select Your Prefered Communication Channel'} </label>
      <div class="radio-button-wrapper">
          <div>
              ...
              <span data-value="${placeholders.contactUsChannelEmail || 'Email'}">${placeholders.contactUsChannelEmail || 'Email'}</span>
          </div>
          <div>
              <span class="icon icon-radio-icon communication-option"></span>
              <span data-value="${placeholders.contactUsChanelMobile || 'Mobile'}">${placeholders.contactUsChanelMobile || 'Mobile'}</span>
          </div>
        </div>
     </div>
  </div>
...
<div class="input-field-wrapper phonevalidation notransistion">
  <label for="phone">${placeholders.contactUsMobileNumber || 'Mobile Number'}</label>
    <div class="input-wrapper input-field input-phone-prefix">             
      ...
      <input type="tel" id="mobile" name="mobile" placeholder=" " autocomplete="tel" data-phone-prefix="${countryPrefix}" data-country-code="${countryCode}" aria-label="${placeholders.contactUsMobileNumber || 'Mobile Number'}" pattern='${pattern}' maxlength='${maxLength}' required>
    </div>
</div>
<div class="input-field-wrapper">
    <div class="input-wrapper input-field">
...
<div class="input-field-wrapper">
  ...
  const handleTypeSelection = (selectedValue) => {
    ...
    typeSelect?.addEventListener('click', (e) => {
      if (e.target.dataset.value === '') {
        ...
        form.querySelector('.feedback-reason .error-message-container')?.classList.add('hide');
      }
    });
  ...
  feedbackSelect?.addEventListener('click', (event) => {
    ...
    if (selectedValue === '') {
      ...
      form.querySelector('.feedback-type .error-message-container')?.classList.add('hide');
  ...
  phoneInput.addEventListener('input', async () => {
    await validatePhoneInput(phoneInput, placeholders);
  });
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const validateDropdown = validateAddressDropdowns(form, placeholders);
    if (!validateForm(form, placeholders)) {
      return;
    }
    if (!await validatePhoneInput(phoneInput, placeholders)) {
      return;
    }
    if (!validateDropdown) {
      return;
    ...
    variables.content.name = await sanitizeDOM(`${data.get('firstname')} ${data.get('lastname')}`);
    ...
    variables.content.ordernumber = await sanitizeDOM(data.get('ordernumber'));
    variables.content.missingitems = await sanitizeDOM(data.get('missingitems'));
    variables.content.message = await sanitizeDOM(data.get('message'));
