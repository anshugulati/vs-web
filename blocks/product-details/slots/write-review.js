...
import { dataLayerBVFeatureSortClick } from '../../../scripts/analytics/google-data-layer.js';
...
function decorateProduct($parent, product) {
  ...
  const normalizedSrc = product.image.startsWith('//') ? `https:${product.image}` : product.image;
  const image = createOptimizedPicture(normalizedSrc, product.name);
...
function decorateProductReviewFields($parent, sku, bvConfig, bvWriteConfig, placeholders) {
  ...
  if (enableWriteReview && enableWriteReview !== 'false') {
    ...
    buttonContainer.append(continueTo2);
...
function decorateYourInformationFields(yourInformationContainer, sku, customerData, _bvConfig, bvWriteConfig, placeholders, isVerifiedPurchaser) {
  ...
  if (enableWriteReview && enableWriteReview !== 'false') {
    const buttonContainer = document.createElement('div');
    buttonContainer.classList.add('buttons-container');
    buttonContainer.append(continueTo3);
    yourInformationContainer.appendChild(buttonContainer);
  } else {
...
function showNetworkErrorScreen(modal, placeholders) {
  const {
    networkErrorTitle, networkErrorMessage,
  } = placeholders;

  const errorScreen = document.createElement('div');
  errorScreen.classList.add('network-error-screen');
  ...
  errorTitle.textContent = networkErrorTitle;
  errorContent.appendChild(errorTitle);

  const errorMessage = document.createElement('span');
  errorMessage.textContent = networkErrorMessage;
  ...
  modal.querySelector('.image-preview').innerHTML = '';
  modal.querySelector('.image-preview').appendChild(errorScreen);
}

function uploadImage(input, modal, placeholders, bvConfig) {
  ...
  uploadPhoto(input.files[0]).then((response) => {
    if (!response?.data?.HasErrors && response.success) {
      // success scenario
      ...
      if (response?.data?.Errors) {
        ...
        showNetworkErrorScreen(modal, placeholders);
      }
    }
    imageField.classList.remove('loader');
  });
...
function decorateProductImagesFields(productImagesContainer, bvConfig, bvWriteConfig, placeholders) {
  ...
  termsLink.href = basicConfig?.write_review_tnc
    ? `/${lang}/${basicConfig.write_review_tnc}`
    : '#';
...
function showSuccessScreen(modal, locale, placeholders) {
  ...
  if (enableWriteReview && enableWriteReview !== 'false') {
    ...
    successScreen.appendChild(buttonContainer);
  } else {
...
async function handleFormSubmitEvent(event, modal, product, bvConfig, locale, placeholders, fromModal) {
  ...
  if (isValid) {
    ...
    if (response.success && !response.data.HasErrors) {
      ...
      // DL for rating inputs submits on reviews modal on PDP
      dataLayerBVFeatureSortClick('BV_featuresubmitClick', product?.gtmAttributes?.id, 'submit', 'review', 'pdp');
...
function bindEvents(modal, product, bvConfig, bvWriteConfig, locale, placeholders, fromModal) {
  ...
  // Submit step 3
  modal.querySelector('.write-review-form-container button.submit-review').addEventListener('click', (event) => {
    handleFormSubmitEvent(event, modal, product, bvConfig, locale, placeholders, fromModal);
  });
  ...
  modal.querySelector('.write-review-form-container .image-field input').addEventListener('change', (event) => uploadImage(event.target, modal, placeholders, bvConfig));
}
...
function loadDeviceFingerprint() {
  ...
  import('../../../scripts/third-party/iovation-loader.js');
}
...
export default async function decorate(product, fromModal, placeholders, locale, isVerifiedPurchaser = false) {
  ...
  await loadCSS(`/blocks/product-details/${getBrandPath()}write-review.css`);
