...
import {
  dataLayerBVTrackPageView, dataLayerBVFeatureLinkClick, dataLayerBVFeatureSortClick,
  dataLayerBVTrackInView,
} from '../../../scripts/analytics/google-data-layer.js';

const enableWriteReview = await getConfigValue('enable-write-review');
...
export async function decorateReview(reviewInfo, bvConfig, index, locale, placeholders) {
  ...
  if (enableWriteReview && enableWriteReview !== 'false') {
    const ratingsCount = document.createElement('span');
    ratingsCount.classList.add('pdp-product__ratings--details-count');
    ratingsCount.textContent = `${Rating.toFixed(1)}`;
    ratingsItemContainer.appendChild(ratingsCount);
  }
...
async function bindReviewItemsEvents(modal, reviewsResults, placeholders) {
  ...
  modal.addEventListener('click', (event) => {
    const { target } = event;
    // If click occurs inside an element with class 'pdp-product__ratings--helpful-yes'
    if (target.closest('.pdp-product__ratings--helpful-yes')) {
      const reviewElement = target.closest(
        '.pdp-product__ratings--helpful-yes',
      );
      const reviewId = reviewElement.getAttribute('data-review-id');

      // DL for positive button click
      if (!reviewElement.dataset.handlerAttached) {
        dataLayerBVFeatureSortClick(
          'BV_featurehelpfulnessClick',
          reviewId || reviewsResults?.[0]?.ProductId,
          'helpfulness',
          'positive',
          'review',
        );
        handleReviewHelpfulEvent(event);
        reviewElement.dataset.handlerAttached = 'true';
      }
    } else if (target.closest('.pdp-product__ratings--helpful-no')) {
      // If click occurs inside an element with class 'pdp-product__ratings--helpful-no'
      const reviewElement = target.closest('.pdp-product__ratings--helpful-no');
      const reviewId = reviewElement.getAttribute('data-review-id');

      // DL for negative button click
      if (!reviewElement.dataset.handlerAttached) {
        dataLayerBVFeatureSortClick(
          'BV_featurehelpfulnessClick',
          reviewId || reviewsResults?.[0]?.ProductId,
        ...
        handleReviewHelpfulEvent(event, false);
        reviewElement.dataset.handlerAttached = 'true';
      }
    } else if (target.closest('.pdp-product__ratings--helpful-report')) {
      // If click occurs inside an element with class 'pdp-product__ratings--helpful-report'
      const reviewElement = target.closest(
        '.pdp-product__ratings--helpful-report',
      );
      const reviewId = reviewElement.getAttribute('data-review-id');

      // DL for reported button click
      dataLayerBVFeatureSortClick(
        'BV_featurehelpfulnessClick',
        reviewId || reviewsResults?.[0]?.ProductId,
        'helpfulness',
      ...
      handleReviewReportEvent(event);
    } else if (target.closest('.pdp-product__bottom-badges--translate')) {
      // Translate and translated badge events
      const index = Array.from(
        modal.querySelectorAll('.pdp-product__bottom-badges--translate'),
      ).indexOf(target.closest('.pdp-product__bottom-badges--translate'));
      handleGoogleTranslatelEvent(event, true, reviewsResults[index]);
    } else if (target.closest('.pdp-product__bottom-badges--translated')) {
      const index = Array.from(
        modal.querySelectorAll('.pdp-product__bottom-badges--translated'),
      ).indexOf(target.closest('.pdp-product__bottom-badges--translated'));
      handleGoogleTranslatelEvent(event, false, reviewsResults[index]);
    }
  });
...
async function handleSortChangeEvent(event, modal, sku, bvConfig, locale, placeholders) {
  ...
  // eslint-disable-next-line camelcase
  const { reviews_on_loadmore } = bvConfig.basic;
  // DL for sort button selection functionality for review modal on PDP
  dataLayerBVFeatureSortClick('BV_featuresortClick', sku, 'sort', 'Featured', sort);
...
async function bindReviewsEvents(modal, reviews, ctx, bvConfig, locale, placeholders) {
  ...
  if (modal.querySelector('.pdp-product__ratings--load-more')) {
    modal.querySelector('.pdp-product__ratings--load-more').addEventListener('click', (event) => {
      handleLoadMoreEvent(event, reviews, sku, bvConfig, locale, placeholders);
      // DL for loadmore button on review  modal on PDP
      dataLayerBVFeatureLinkClick('BV_featureloadmoreClick', reviews, ctx?.data?.gtmAttributes, 'loadmore', 'reviews', '');
    });
  }
  ...
  modal.querySelector('.pdp-product__ratings--write-review button').addEventListener('click', (event) => {
    ...
    modal.close();
    // DL for rating input form modal on PDP
    dataLayerBVFeatureLinkClick('BV_featurewriteClick', reviews, ctx?.data?.gtmAttributes, 'write', 'review', 'PrimaryRatingSummary');
  });
...
async function viewRatingsOverlay(ctx, placeholders, sku, reviews, bvConfig, locale) {
  ...
  if (enableWriteReview && enableWriteReview !== 'false') {
    ...
    ratingsDiv.classList.add('pdp-product__ratings--average---top');
    const ratingdivCount = document.createElement('div');
    ratingdivCount.classList.add('pdp-product__ratings--average---count');
    ratingdivCount.textContent = `${AverageOverallRating.toFixed(1)}`;

    const ratingdivCountLabel = ratingReviewCount || 'Total {{count}} Reviews';
    ratingsCount.textContent = `${ratingdivCountLabel.replace('{{count}}', TotalReviewCount)}`;
    ratingsDiv.appendChild(ratingdivCount);
    ratingdivCount.appendChild(ratingsContainer);
    ratingsContainer.appendChild(ratingsCount);
    ratingsDiv.appendChild(ratingsContainer);
    ratingsSummary.appendChild(ratingsDiv);
  } else {
  ...
  createModalFromContent('view-ratings-dialog', titleDiv.textContent, ratingsOverlayContent.outerHTML, ['pdp-modal', 'pdp-modal--ratings'])
    .then(async () => {
      if (isLoggedInUser()) {
        ...
        const reviewModal = document.querySelector('#view-ratings-dialog .pdp-modal--ratings .modal-content ');
        ...
        if (reviewModal) {
          let hasScrolledBeyond200 = false; // Flag for scrollTop >= 70
          reviewModal.addEventListener('scroll', () => {
            const { scrollTop } = reviewModal;
            const targetReviewItemCont = reviewModal.querySelector('.pdp-product__ratings--details-items');
            if (reviewModal && targetReviewItemCont) {
              // Check for scrollTop >= 70 1st review on screen
              if (scrollTop >= 70 && !hasScrolledBeyond200) {
                hasScrolledBeyond200 = true; // Set the flag to true
                // DL for review or rating scroll BV_trackInView event for review modal on PDP
                dataLayerBVTrackInView('BV_trackInView', sku);
                dataLayerBVTrackInView('BV_trackViewedCGC', sku);
              } else if (scrollTop < 70) {
                hasScrolledBeyond200 = false; // Reset the flag if scrolled back up
              }
            }
          });
        }
...
export async function decorateRatings(ctx, ratingsContainer, placeholders, locale) {
  getBVConfig().then((bvConfig) => {
    ...
    getReviews(ctx.data.sku, 0, reviews_initial_load).then(async (reviews) => {
      // DL on PDP page load
      dataLayerBVTrackPageView(reviews, ctx);
      const { sku } = ctx.data;
      ...
      if (reviews?.TotalResults > 0
        ...
        ratingsLink.addEventListener('click', (e) => {
          ...
          // DL for view all reviews link  on PDP
          dataLayerBVFeatureLinkClick('BV_featurelinkClick', reviews, ctx?.data?.gtmAttributes, 'link', 'review_count', 'PrimaryRatingSummary');
        });
        ratingsCount.addEventListener('click', (e) => {
          e.preventDefault();
          openModal('view-ratings-dialog');
          // DL for view all reviews count click on PDP
          dataLayerBVFeatureLinkClick('BV_featurelinkClick', reviews, ctx?.data?.gtmAttributes, 'link', 'review_count', 'PrimaryRatingSummary');
        });
        loadCSS(`/blocks/product-details/${getBrandPath()}ratings.css`);
      } else {
        const authorReviews = await getStoredMyReview(sku);
        const authorEmail = await getCustomer();
