...
export function setDefaultSizeGuideFilters(options) {
...
function appendSizeGuideSelectorSkeleton() {
  ...
  if (!document.querySelector('.size-guide-selectedFilters')) {
    let selectedCategoryTabId;
    if (defaults.options) {
      const tabIdsArray = Array.from(document.querySelectorAll('.tabs-tab')).flatMap((button) => button.getAttribute('data-tab-ids')?.split(',').map((id) => id.trim()));
      if (tabIdsArray.includes(defaults.options.primaryCategory)) {
        selectedCategoryTabId = defaults.options.primaryCategory;
      } else if (tabIdsArray.includes(defaults.options.secondaryCategory)) {
        selectedCategoryTabId = defaults.options.secondaryCategory;
      }
    }
    // extracting the target tab and category from the URL
    const urlParams = new URLSearchParams(window.location.search);
    const tabSelection = urlParams.get('tab')?.toLowerCase() || selectedCategoryTabId; // L1 selection
    const categorySelection = urlParams.get('category')?.toLowerCase() || ''; // L2 selection
...
function autoSelectFiltersBasedOnPDP() {
  ...
  if (sizeGuideTab) {
    const tabButton = Array.from(document.querySelectorAll('.tabs-tab')).find((button) => {
      const tabIds = button.getAttribute('data-tab-ids')?.split(',')?.map((id) => id.trim());
      return tabIds?.includes(sizeGuideTab) ? button : undefined;
    });

    const tab = tabButton || document.querySelector('.tabs-tab');
    if (tab.getAttribute('data-tab-ids').indexOf(sizeGuideTab.toLowerCase()) < 0) sizeGuideCategoryFilter = undefined;
    if (tab) {
      tab.click();
