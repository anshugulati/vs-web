...
async function login(form, redirectUrl) {
  ...
  try {
    ...
    if (!response.ok && data.message) {
      const accountNotConfirmedMessage = 'This account isn\'t confirmed. Verify and try again.';
      // As per backend message will always be the same so we have matched this to give
      // respective error message
      const loginErrorMessage = (data.message === accountNotConfirmedMessage)
        ? (placeholders.verifyLoginErrorMessage || 'Your account has not been activated or is blocked')
        : (placeholders.loginErrorMessage || 'Unrecognized email address or password.');
    ...
    datalayerLogin('Login Success', 'Email : valid', dlPayload);
