...
import { customSelectbox } from '../../scripts/forms.js';
  ...
  itemRows.forEach((row) => {
    ...
    if (returnItemsObj[itemId]) {
      ...
      if (returnQuantityInput && returnItemsObj?.[itemId]?.isPromotion) {
        returnQuantityInput.parentElement.classList.add('disabled');
...
const returnReasonSelectHandler = (event) => {
  const { itemId } = event.target.parentElement.dataset;
  if (!itemId) {
    return;
  }

  const reasonCode = event.target.dataset.value;
...
const returnQuantitySelectHandler = (event) => {
  ...
  const quantity = event.target.dataset.value;
...
const confirmRefundButtonHandler = async () => {
  ...
  } = await postCreateReturn({ rmaDataObject });
...
const getEGiftRefundMethodContent = async (
  ...
  if (isEgiftRefundDisabled) {
    // if E-Gift is disabled, hide the main description
...
const renderHybridPaymentDescriptions = (wrapper, placeholders) => {
  const hybridPaymentMainText = document.createElement('div');
  hybridPaymentMainText.classList.add('return-refund-main-description');
  hybridPaymentMainText.innerText = placeholders?.orderHybridPaymentMainTitle || 'Original Multiple payment methods used';
  wrapper.appendChild(hybridPaymentMainText);

  const hybridPaymentSubText = document.createElement('div');
  hybridPaymentSubText.classList.add('return-refund-sub-description');
  ...
  wrapper.appendChild(hybridPaymentSubText);
};
...
const getRefundMethodsContent = async (order, placeholders) => {
  ...
  // Checks if the E-gift refund method is disabled from the config or not
  const refundMethodEgiftEnabled = await getConfigValue('refund-method-egift-enabled');
  const isEgiftRefundDisabled = refundMethodEgiftEnabled !== 'true';
  ...
  if (refundMethod === 'checkout_com_upapi') {
    ...
    // Check if E-Gift is part of hybrid payment
    ...
    if (isHybrid) {
      ...
      renderHybridPaymentDescriptions(refundMethodsContent);
    }

    if (hasHybridEgift) {
      const eGift = await getEGiftRefundMethodContent(order, placeholders, showRadioInput, (data) => updateProcessedRmaDataObject('extension_attributes', data), isHybrid, isEgiftRefundDisabled);
      if (eGift) {
        refundMethodsContent.appendChild(eGift);
      }
    } else if (isEgiftRefundDisabled) {
      showRadioInput = false;
      updateProcessedRmaDataObject('extension_attributes', {});
    } else {
      ...
      if (eGift) {
        refundMethodsContent.appendChild(eGift);
      } else { // Don't show radio button if the refund method is credit / debit card.
        showRadioInput = false;
      }
    }

    // If Aura is part of hybrid refund method
    if (hasHybridAura) {
  ...
  if (
    ...
    const eGift = await getEGiftRefundMethodContent(order, placeholders, false, (data) => updateProcessedRmaDataObject('extension_attributes', data), false, isEgiftRefundDisabled);
  ...
  if (refundMethod === 'hps_payment') {
    ...
    if (isHybrid) {
      renderHybridPaymentDescriptions(refundMethodsContent, placeholders);
    }
...
const renderRefundPickupAddress = async (order, placeholders) => {
  ...
  const refundMethodTitleMain = document.createElement('div');
...
const buildOrderReturnList = async (
  ...
  items.forEach(async (item) => {
    ...
    returnReasons.classList.add('return-reasons-column', 'return-config-column', 'dropdown', 'hidden');
    ...
    const defaultSelected = document.createElement('div');
    defaultSelected.classList.add('select-selected');
    defaultSelected.innerText = placeholders.ordersChooseAReason || 'Choose a reason';
    customSelect.appendChild(defaultSelected);
    const returnReasonOptions = document.createElement('div');
    returnReasonOptions.classList.add('return-reasons-options', 'select-items', `return-reasons-options-${item.item_id}`);
    ...
    const defaultReasonOption = document.createElement('div');
    defaultReasonOption.setAttribute('data-value', '');
    defaultReasonOption.setAttribute('disabled', true);
    ...
    if (returnsConfig?.return_reasons?.length) {
      returnsConfig.return_reasons.forEach((reason) => {
        ...
        option.setAttribute('data-value', reason.id);
        option.innerText = reason?.label || '';
        returnReasonOptions.appendChild(option);
      });
    }

    customSelect.appendChild(returnReasonOptions);
    customSelectbox(customSelect);
    customSelect.querySelectorAll('.select-items div').forEach((divItem) => {
      divItem.addEventListener('click', returnReasonSelectHandler);
    });

    returnReasons.appendChild(reasonTitle);
    returnReasons.appendChild(customSelect);

    // Select quantity
    const returnQuantity = document.createElement('div');
    returnQuantity.classList.add('return-quantity-column', 'return-config-column', 'dropdown', 'hidden');
    ...
    selectQuantity.classList.add('custom-select');
    const defaultQuantity = document.createElement('div');
    defaultQuantity.classList.add('select-selected');
    defaultQuantity.innerText = 1;
    selectQuantity.appendChild(defaultQuantity);
    const returnQuantityOptions = document.createElement('div');
    returnQuantityOptions.classList.add('return-quantity-options', 'select-items', `return-quantity-options-${item.item_id}`);
    ...
    Array.from({ length: itemQuantity || 1 }).forEach((_, qty) => {
      const quantityOption = document.createElement('div');
      quantityOption.setAttribute('data-value', qty + 1);
      quantityOption.innerText = qty + 1;
      returnQuantityOptions.appendChild(quantityOption);
    });

    selectQuantity.appendChild(returnQuantityOptions);
    customSelectbox(selectQuantity);
    selectQuantity.querySelectorAll('.select-items div').forEach((divItem) => {
      divItem.addEventListener('click', returnQuantitySelectHandler);
    });

    returnQuantity.appendChild(returnQuantityTitle);
    returnQuantity.appendChild(selectQuantity);
...
export const renderReturnFooter = (order, footerBottom, placeholders) => {
  ...
  orderDetailsLink.setAttribute('href', `/${lang}/user/account/orders/details?orderId=${order?.increment_id}`);
...
const renderOrderReturns = async (block, incrementId, placeholders, locale) => {
