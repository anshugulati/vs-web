...
export async function getAddressRegionSegments(countryId) {
  const attributeId = 'address_region_segment';
...
export async function createAddress(addressData, config = {}) {
  ...
  const defaultPrimary = await getConfigValue('default-primary-address');
  const defaultPrimaryAddress = defaultPrimary ? defaultPrimary === 'true' : true;
  const variables = { address: addressData };
  variables.address.default_billing = defaultPrimaryAddress;
