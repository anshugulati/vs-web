...
export const FNAME_PATTERN = String.raw`^[A-Za-z]+$`;
export const LNAME_PATTERN = String.raw`^[A-Za-z]+$`;
...
export function showPageErrorMessage(message, source) {
  ...
  // error message DL on checkout if anything goes wrong
...
export function getErrorMessageForField(fieldName, fieldLabel, placeholders, errorType = 'required', fieldType = 'text', chars = 0, chars1 = 0) {
  ...
  if (!errorMessage) {
    if (errorType === 'required') {
      ...
      errorMessage = placeholders.egiftErrorMsgAmountTopUp || 'Please enter amount in the range of {{chars}} to {{chars1}}';
    }
  }
  const minPrice = placeholders.minMonetaryValue;
  const maxPrice = placeholders.maxMonetaryValue;
  return errorMessage ? errorMessage.replace('{{label}}', fieldLabel).replace('{{minPrice}}', minPrice).replace('{{maxPrice}}', maxPrice).replace('{{chars}}', chars)
    .replace('{{chars1}}', chars1)
...
export function setErrorMessageForField($field, placeholders) {
  ...
  if ($field.getAttribute('min') && $field.getAttribute('max')) {
    $field.dataset
      .validationPriceRangeMessage = getErrorMessageForField($field.name, title, placeholders, 'priceRange', $field.type, $field.min, $field.max);
  }

  setErrorForField($field);
}

export function validateAddressDropdowns(form, placeholders, isCheckoutPage) {
  ...
  form.querySelectorAll('.custom-select').forEach((dropdown) => {
    ...
    if (!selectionBox.getAttribute('data-value')) {
      let errorMessage = placeholders.errorRequired || `Please select ${dropdown.getAttribute('aria-label')}`;
      errorMessage = errorMessage.replace('{{label}}', dropdown.getAttribute('aria-label'));
      showErrorMessage(dropdown, errorMessage);
      selectedFromDropdown = false;
    } else {
...
export async function isDateBefore(dateString, compareDateString) {
  ...
  return date >= minDate && date < compareDateString;
}
...
export async function dateValidation(dateInput, placeholders) {
  ...
  if (DATE_PATTERN.test(inputValue)) {
    ...
    if (!isValidDateBefore) {
      showErrorMessage(dateInput, `${placeholders.dobRangeError} ${dateformat}.`);
      if (targetButton) {
...
export function selectItemsTrigger(form) {
  form.querySelectorAll('.select-items div').forEach((optionitem) => {
