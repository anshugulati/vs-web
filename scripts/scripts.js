...
const PRE_RENDERED_PAGES = ["product detail page", "product listing page"];
...
async function loadBlock(block) {
  ...
  if (status !== "loading" && status !== "loaded") {
    ...
    try {
      ...
      if (blockName === "banner-collection") {
        cssPath = `${window.hlx.codeBasePath}/blocks/vic/${blockName}/${blockName}.css`;
      } else {
        cssPath = `${window.hlx.codeBasePath}/blocks/${blockName}/${getBrandPath()}${blockName}.css`;
      }
      ...
      const decorationComplete = new Promise((resolve) => {
        (async () => {
          try {
            let jsPath = "";
            if (blockName === "banner-collection") {
              jsPath = `${window.hlx.codeBasePath}/blocks/vic/${blockName}/${blockName}.js`;
            } else {
              jsPath = `${window.hlx.codeBasePath}/blocks/${blockName}/${blockName}.js`;
            }
...
export async function createModal(
  ...
  const titleIconClassValue = titleIconClass
    ? `icon icon-${titleIconClass} icon-title-left`
    : "";
  ...
  dialog.addEventListener("click", (event) => {
    const stickyActionFooter = document.querySelector(
      ".pdp-product__actions.button-bar-wishlist"
    );
...
export function isAruaLinked() {
  const auraCommonData = JSON.parse(
    localStorage.getItem("aura_common_data") || "{}"
  );
...
async function decorateScrollToTop(main) {
  ...
  const placeholders = await fetchPlaceholders(
    `${urlPrefix + document.documentElement.lang}`
  );
...
export function createOptimizedPicture(
  ...
  if (!isAbsoluteUrl)
    return libCreateOptimizedPicture(
      "/icons/fallback.svg",
      alt,
      eager,
      breakpoints
...
async function decorateExternalImages(ele, deliveryMarker) {
  ...
  extImages.forEach((extImage) => {
    if (
      isExternalImage(extImage, deliveryMarker) &&
      !extImage.classList.contains("hide")
    ) {
      const isDMOpenAPI = isDMOpenAPIUrl(extImage.getAttribute("href"));
      const extImageSrc = createOptimizedSrc(
        extImage.getAttribute("href"),
        isDMOpenAPI
      );
      ...
      extPicture.querySelectorAll("source, img").forEach((child) => {
        if (child.tagName === "SOURCE") {
          ...
          if (srcset) {
            const queryParams = appendQueryParams(
              new URL(srcset, extImageSrc),
              searchParams
            );
    ...
    templates.forEach(async (template) => {
      if (availableTemplates.includes(template)) {
        ...
        const decorator = await Promise.all([
          ...
          loadCSS(
            `${window.hlx.codeBasePath}/templates/${templateName}/${getBrandPath()}${templateName}.css`
          ),
...
const REGEX_PLACEHOLDER = /\{\{([A-Za-z\-0-9]+)\}\}/gi;
  ...
  if (path && path.startsWith("/")) {
    ...
    if (resp.ok) {
      ...
      const resetAttributeBase = (tag, attr) => {
        mainDiv
          .querySelectorAll(`${tag}[${attr}^="./media_"]`)
          .forEach((elem) => {
            elem[attr] = new URL(
 ...
 */
export async function createModalFromFragment(
  id,
  title,
  fragmentUrl,
  classList
 ...
 */
// eslint-disable-next-line max-len
export async function createModalFromContent(
  id,
  title,
  content,
  classList,
...
function onDecoratedElement(fn) {
  ...
  const observer = new MutationObserver((mutations) => {
    ...
    if (
      mutations.some(
        (m) =>
          (m.target.dataset.blockName === "recommendations" ||
...
export async function getAndApplyRenderDecisions(
  ...
  const { xdmPayload, EVENT_QUEUE, targetDisplayEvent } = await import(
    "./target/target-events.js"
  );
  ...
  try {
    ...
    const customPropositions = propositions?.filter(
      (p) => p.scope !== "__view__"
    );
...
function getLCPSection() {
  return (
    document.querySelector("[data-lcp-section]") ??
...
async function getTargetBlocksMetadata() {
  ...
  const personalizedBlocks = document.querySelectorAll(
  ...
  personalizedBlocks?.forEach((el) => {
    if (el.dataset.tabUrl && shouldFetchFragments) {
      fetchPromises.push(
        fetchData(`${el.dataset.tabUrl}.plain.html`, {
          headers: {
            "Cache-Control": "max-age 60",
          },
        })
  ...
  try {
    const responses =
      window.tabUrlFragments?.length > 0
        ? window.tabUrlFragments
...
const alloyLoadedPromise = (datastreamId, orgId) =>
  initWebSDK("./alloy.js", {
    datastreamId,
    orgId,
...
export function formatPDPUrlAttributes(url, src = "main") {
  ...
  const heightDimension =
    src === "main"
      ? placeholders.productZoomMedium || 630
      : placeholders.productZoomLarge || 800;
  let formatUrl = url
    .replace(/width=\d+&?/g, "")
    .replace(/height=\d+&?/g, "")
    .replace("/original/", "/");
  formatUrl = formatUrl.includes("?")
    ? `${formatUrl}&height=${heightDimension}`
    : `${formatUrl}?height=${heightDimension}`;
  return formatUrl;
}
...
export function getProductAssetsData(assetType, product) {
  ...
  if (assetType === "assets_pdp") {
    ...
    const imageStyles = product?.attributes.find(
      (attr) => attr.name === "image_styles"
    )?.value;
    if (imageStyles) {
...
async function decorateMetaTitle() {
  ...
  fetchPlaceholdersForLocale().then((placeholders) => {
    ...
    if (lang === "ar") {
      metaTitle.innerText = titleSuffix
...
async function loadBreadcrumb(breadcrumbWrapper) {
  if (breadcrumbWrapper) {
    const decorator = await Promise.all([
      ...
      loadCSS(
        `${window.hlx.codeBasePath}/blocks/breadcrumb/${getBrandPath()}breadcrumb.css`
      ),
    ]).then(([mod]) => mod.default);
    if (decorator) {
...
function optimizeFragmentLoading() {
    ...
    if (blockName === "fragment") {
      ...
      fragments.push(
        loadFragment(path).then((fragment) => {
          window.preloadedFragments[path] = fragment;
        })
      );
    }
  });
  ...
  if (fragmentLCPBlockHint.length > 0) {
    ...
    return Promise.all([
      ...
      ...fragmentLCPBlockHint.map((name) =>
        loadCSS(`/blocks/${name}/${brandPath}${name}.css`)
      ),
      ...fragmentLCPBlockHint.map(
        (name) => import(`/blocks/${name}/${name}.js`)
      ),
      ...fragments,
...
async function decoratePlaceholders() {
  ...
  const isSupportedTemplate =
    DYNAMIC_PLACEHOLDER_TEMPLATE_LIST.filter((value) =>
      templateList.includes(value)
    ).length > 0;

  if (
    dynamicKeys &&
    dynamicKeys.toLowerCase() === "true" &&
    isSupportedTemplate
  ) {
    const placeholders = await fetchPlaceholdersForLocale();
...
function getStickyElements() {
  if (mobileDevice.matches) {
    STICKY_ELEMENTS = document.querySelectorAll(
      ".sticky-element.sticky-mobile"
    );
  } else {
    STICKY_ELEMENTS = document.querySelectorAll(
      ".sticky-element.sticky-desktop"
    );
  }
}
...
export function enableStickyElements() {
  ...
  window.addEventListener("scroll", () => {
    ...
    STICKY_ELEMENTS.forEach((element, index) => {
      ...
      if (
        currentScrollPosition < LAST_SCROLL_POSITION &&
        currentScrollPosition <= offsets[index]
      ) {
    ...
    if (stackedHeight !== LAST_STACKED_HEIGHT) {
      ...
      document
        .querySelector(":root")
        .style.setProperty("--stacked-height", `${stackedHeight}px`);
    }
  });
}
...
async function loadLazy(doc) {
  ...
  const enableGTM = async () => {
    try {
      const { loadGTM = () => {} } = DISABLE_MARTECH
        ? {}
        : await import("./analytics/google-data-layer.js");
  ...
  if (
    getLCPSection().getAttribute("data-section-status") === "loaded" &&
    !isMobileApp()
  ) {
    loadHeader(doc.querySelector("header"));
  } else {
    observer.observe(getLCPSection(), {
      attributes: true,
      attributeFilter: ["data-section-status"],
    });
  }

  await Promise.all([loadBlocks(main), enableGTM()]);
...
export async function fetchIndex(
  ...
  const handleIndex = async (offset) => {
    const resp = await fetch(
      `/${lang}${indexFile}.json?limit=${pageSize}&offset=${offset}`
    );
  ...
  const handleGlobalIndex = async (offset) => {
    const respGlobalPromise = fetch(
      `/${lang}global-${indexFile}.json?limit=${pageSize}&offset=${offset}`
    );
    const respPromise = fetch(
      `/${lang}${indexFile}.json?limit=${pageSize}&offset=${offset}`
    );

    const [respGlobal, resp] = await Promise.all([
      respGlobalPromise,
      respPromise,
    ]);
  ...
  if (hasGlobal) {
    window.index[indexFile].promise = handleGlobalIndex(
      window.index[indexFile].offset
    );
  } else {
    window.index[indexFile].promise = handleIndex(
      window.index[indexFile].offset
    );
  }
...
export async function showCommerceErrorPage(code = 404) {
  const { pageLoadData = () => {} } = DISABLE_MARTECH
    ? {}
  ...
  document
    .querySelector(".algolia-product-listing-container")
...
export function getPreviewPreaccessType(previewPreaccessData, lang) {
  ...
  const extractPdpText = (pdpText) =>
    typeof pdpText === "object" ? pdpText[lang] : pdpText || null;

  const toISODate = (dateStr) =>
    dateStr ? new Date(dateStr.replace(" ", "T")).toISOString() : null;

  if (
    previewPreaccessData.preaccess_sd &&
    previewPreaccessData.preaccess_ed &&
  ...
  if (
    previewPreaccessData.preview_sd &&
    previewPreaccessData.preview_ed &&
...
export async function getCustomerTier() {
  try {
    ...
    if (custResponse?.custom_attributes) {
      const tierAttribute = custResponse.custom_attributes.find(
        (attribute) => attribute.attribute_code === "tier_name"
      );
...
window.handleShoeSizeRecommendation = function handleShoeSizeRecommendation(
  recommendation
) {
  const recommendedSize =
    recommendation?.size?.eu &&
  ...
  const sizes = document.querySelectorAll(
    ".size_shoe_eu .dropin-text-swatch__label"
  );
...
export async function createShoeAi() {
  ...
  const [domainProd, domainStage, customer, shopId, shoeAiMode] =
    await Promise.all([
...
(() => {
  if (!window.location.pathname?.includes("/checkout"))
    sessionStorage.removeItem("digital-cart-id");
})();
