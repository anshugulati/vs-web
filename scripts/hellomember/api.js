...
export async function getAPCCustomerData(programCode = 'hello_member', sendQueryParams = true) {
  ...
  const url = await createUrl(`customers/apcCustomerData/${customer.id}${sendQueryParams ? `?customerId=${customer.id}&programCode=${programCode}` : ''}`);
...
export async function getAPCTierProgressData(useCache = false, programCode = 'hello_member') {
  ...
  if (useCache && cachedData) {
    ...
    if (isExpired(parsedData.expiryTime)) {
      sessionStorage.removeItem(`apcTierProgressData:${lang}`);
  ...
  sessionStorage.setItem(`apcTierProgressData:${lang}`, JSON.stringify({
    data,
