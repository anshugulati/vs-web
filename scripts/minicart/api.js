  ...
  cartItemsQuantity: {
    watch: (callback) => {
      store.subscribe((cart) => {
        const isCodSurchargeApplied = cart?.cart?.extension_attributes?.surcharge?.is_applied;
        const codSurchargeAmt = cart?.cart?.extension_attributes?.surcharge?.amount;
        const baseGrandTotal = cart?.totals?.base_grand_total;
        const grandTotal = isCodSurchargeApplied && codSurchargeAmt
          ? baseGrandTotal - codSurchargeAmt
          : baseGrandTotal;
        const deliveryAmountFee = cart?.totals?.shipping_incl_tax;
        const grandTotalWithoutCharge = grandTotal - deliveryAmountFee;
        callback(cart.cart?.items_qty || 0, grandTotalWithoutCharge || 0, cart?.totals?.base_currency_code || 'AED');
      });
    },
  },
};
