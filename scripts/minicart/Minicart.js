...
react/no-unused-state, react/destructuring-assignment */
import {
  ...
  render(props, state) {
    ...
    return html`<div class="minicart-panel">
      <div class="minicart-header">
        ...
        <div class="subtotal">Sub-Total: <span class="price">${this.formatter.format(cart.totals.subtotal_excluding_tax.value)}</span></div>
        <button class="close" onClick=${() => close(false)}>Close</button>
      </div>
...
export async function toggle(refetch = true) {
  ...
  render(app, document.querySelector('.minicart-wrapper'));
}
