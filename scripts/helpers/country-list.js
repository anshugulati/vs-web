...
export async function getMaxLengthByCountryCode(countryCode) {
  const countryByCode = countryList.find(countryByCode => countryByCode.code === countryCode);
  return countryByCode.phoneLength;
}

export async function getCountryIso(countryCode) {
  const countryIso = countryList.find(country => country.code === countryCode);
  return countryIso.phone;
}
