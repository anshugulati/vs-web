...
export const productDetailArQuery = `query ProductQuery($sku: String!) {
  products(skus: [$sku]) {
    ...productViewFields
  }
  products_en(skus: [$sku]) {
    ...productViewFields
  }
}
${productViewFragment}
...
export const getPromotionByRuleId = `query getPromotions($rule_id: [String]) {
  promotionsEnrichmentData(rule_id: $rule_id) {
    total
    offset
    limit
    data
...
export const productStockBySkuStatus = `query getStockStatusBySku($skus: [String]) {
  commerce_products(filter: { sku: { in: $skus } }, pageSize: 100) {
    items {
      id
      sku
      name
...
// Graphql request commerceCategoryEDS for brands like FL to add features like shop by shoe size.
export const commerceCategoryEDS = `query ($categoryId: String) {
  commerce_categories(
...
export const promotionByUrlKeyQuery = `query($urlKey:String) {
    commerce_promotionUrlResolver(url_key: $urlKey) {
      id
      title
      description
    }
...
export async function cacheProductData(product, variant = false) {
  window.product = window.product || {};

  if (variant) {
    window.product[product.variantSku] = product;
  } else {
...
export async function getProductStockStatus(sku) {
  const { data } = await performAPIMeshGraphQLQuery(productStockBySkuStatus, {
    skus: [sku],
  });
  return data.commerce_products;
}

export async function getProductPromotionByRuleId(ruleId) {
  const { data } = await performAPIMeshGraphQLQuery(getPromotionByRuleId, {
    rule_id: [ruleId],
  });
  return data;
}
...
export function getCachedProductData(sku) {
  const product = window.product?.[sku];
...
export async function getPromotionByUrlKey(urlPath) {
