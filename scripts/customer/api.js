...
class Store {
  ...
  setCustomerLastOrder(lastOrderData) {
    this.customerLastOrder = lastOrderData;
  }
...
export async function getCustomerLastOrder(useCache = true) {
  const token = getSignInToken();
  ...
  // load from store
  if (useCache && store.customerLastOrder) {
    console.log('from cache', store.customerLastOrder);
    return store.customerLastOrder;
  }
  ...
  // if no data, set empty object
  if (data === false) {
    store.setCustomerLastOrder({});
  } else {
    store.setCustomerLastOrder(data);
  }
...
export async function hasOrders() {
  const searchCriteria = 'searchCriteria[pageSize]=1&searchCriteria[currentPage]=1';
...
const HAS_ORDER_FLAG_KEY = 'customerHasOrders';
export function clearOrderFlag() {
  sessionStorage.removeItem(HAS_ORDER_FLAG_KEY);
}

export function setOrderFlag(customerHasOrders) {
  sessionStorage.setItem(HAS_ORDER_FLAG_KEY, customerHasOrders);
}

export function getOrderFlag() {
  return sessionStorage.getItem(HAS_ORDER_FLAG_KEY);
}

export async function hasCustomerOrder() {
  const token = getSignInToken();
  ...
  // load from store
  const customerHasOrder = getOrderFlag();
  if (customerHasOrder) {
    return customerHasOrder;
  }

  // fetch API
  const customerHasOrders = await hasOrders();

  // if no data, set empty object
  if (customerHasOrders !== null) {
    setOrderFlag(customerHasOrders);
  }
