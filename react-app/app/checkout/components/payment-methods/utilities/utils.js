 ...
 *   A promise object.
 */
export const preparePaymentData = (data, paymentCallbackSuccess = '', paymentCallbackError = '' , guestOrderToken = '', reserveOrderId = '') => {
  ...
  if (isUpapiPaymentMethod(paymentData.method)
    ...
    params.payment.additional_data.successUrl = `${window.location.origin}/${getLanguageAttr()}${paymentCallbackSuccess}?token=${guestOrderToken ?? ''}&reserveOrderId=${reserveOrderId ?? ''}`;
...
export async function getCardTokens() {
  ...
  if (allowedCardsMapping) {
    ...
    if (tokenizedCards) {
      let savedCard = [];
      let items = tokenizedCards?.response?.items;
