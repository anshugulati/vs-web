...
const CashOnDelivery = forwardRef(({ finalisePayment }, ref) => {
  ...
  const verifyOTPHandler = async () => {
    ...
    if (response?.response === true) {
      const result = await getSubCartGraphql(isLoggedIn, cartId, [ApiConstants.CART_QUERY__MOBILE_NUMBER_VERIFIED, ApiConstants.CART_QUERY__PRICES]);
      if (result) {
        setOtp('');
        setCart((prevCart) => ({
          ...prevCart,
  ...
  const openMobileChangeDialog = () => {
    ...
    setDeliveryInformation((prev) => ({
      ...prev, isDialogOpen: true, changeAddress: 'shipping', isModalVisible: true, updateOnlyTelephone: true, infoMessage: placeholders.codMobileUpdateInfoText,
    }));
  };
  ...
  const otpOnChangeHandler = (otpStr) => {
    setOtp(otpStr);
    error && setError('');
  };
