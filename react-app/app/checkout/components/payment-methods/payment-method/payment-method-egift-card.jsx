...
import { validateForMaxDecimalDigits } from "../../../../../utils/base-utils.js";
...
const PaymentMethodEGiftCard = forwardRef((props, ref) => {
  ...
  const isDisablePayment = PAYMENT_METHODS_NOT_FOR_EGIFTS.includes(selectedPaymentMethod?.code) || false;
  ...
  useEffect(() => {
    ...
    const fetchCurrency = async () => {
      ...
      if (isEgiftChecked && !isDisablePayment) {
  ...
  useEffect(() => {
    setIsDisableSection((cartData?.hps_redemption_type === AppConstants.PAYMENT_EGIFT_CARD_GUEST) || isDisablePayment);
  }, [methods, cart]);
    ...
    window.dispatchEvent(new CustomEvent(
      'react:datalayerEvent',
      {
        detail: {
          type: 'eGiftCard',
          payload: {
            eventAction: "egift_card_linked",
            eventLabel: event.target.checked ? "checked" : "unchecked"
          }
        },
      }
    ));
    if (event.target.checked) {
      ...
      if (invokeGCRedeem?.response_type) {
        ...
        const isPaymentValid = auraPayment
          ...
          : redeemed_amount === totals.base_grand_total;
