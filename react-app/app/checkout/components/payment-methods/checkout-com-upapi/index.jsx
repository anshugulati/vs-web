...
const CheckoutComUpapi = forwardRef((props, ref) => {
  ...
  const { placeholders, isLoggedIn, setCompletePurchaseLoading, isTopupFlag, configs} = useContext(CartContext);
  const [selectedCard, setSelectedCard] = useState('new');
  const [tokenizedCard, setTokenizedCard] = useState('');
  const [activeCard, setActiveCard] = useState({});
  const [isDialogOpen, setDialogOpen] = useState(false);
  const [savedCards, setSavedCards] = useState([]);
  const setNewlyAddedCardActive = configs?.['set-newly-added-card-as-active'] === 'true';
      ...
      if (tokenizedCardsList instanceof Promise) {
        tokenizedCardsList.then((result) => {
          ...
          if (result?.length) {
            ...
            const activeCardList = setNewlyAddedCardActive ? result[result.length - 1] : result.find((card) => card.isActive);
  ...
  return (
    <>
      {isLoggedIn && savedCards.length > 0 && !isTopupFlag ? (
        <div className="spc-payment-checkout_com">
          ...
          <div className="spc-checkout-card-option">
            ...
            <div>
              <span className="spc-checkout-card-new-card-label" onClick={onClickNewCard} role="button" tabIndex={0} onKeyDown={onClickNewCard}>{placeholders.addNewCardBtnTitle}</span>
              {selectedCard === 'new' && (
                <NewCard
      ...
      <Dialog
        ...
        containerClassName="dialog__checkout-container card-dialog"
      >
