...
function NewCard(props) {
  ...
  const { placeholders, isLoggedIn, isTopupFlag, configs} = useContext(CartContext);
  const allPaymentMethodv2 = configs['checkout-all-payment-methods-v2'] === 'true';
  const enableCreditDebitCardLogo = configs['enable-checkout-cc-dc-input-logo'] === 'true';
  ...
  const showCardType = useCallback((event) => {
    ...
    if (showCardIconOnType) {
      ...
      const cardType = cardNumber.length > 3
        ? getPaymentCardType(cardNumber, checkoutComUpiConfig) : null;
  ...
  const handleBinValidation = (cardNumber) => {
    ...
    const validation = binValidation(cardBin, checkoutComUpiConfig);
  ...
  const renderCardLogo = (item)=>{
    return <CardTypeSVG type={item} />
  }
    ...
    <div className="new-card-wrapper">
      <div className="payment-form-wrapper">
        ...
        <div className="spc-type-textfield spc-type-cc-number">
          ...
          />
          {allPaymentMethodv2 && <label for="spc-cc-number">{placeholders.cardNumberText}</label>}
          {showCardIconOnType && (
            enableCreditDebitCardLogo ? ( <div className={`card-type card-type-${payingCardType}`}>
              {renderCardLogo(payingCardType)}
            </div> ) : <div className={`card-type card-type-${payingCardType}`}/>
          )}
        ...
        <div className="spc-expiry-cvv-column">
          <div className="spc-type-textfield spc-type-expiry">
            ...
            />
            {allPaymentMethodv2 && <label for="spc-cc-expiry">{placeholders.expiryText}</label>}
            <div className="c-input__bar" />
          ...
          <div className="spc-type-textfield spc-type-cvv">
            ...
            />
            {allPaymentMethodv2 && <label for="spc-cc-cvv">{placeholders.cvvText}</label>}
            <div className="c-input__bar" />
            <div id="spc-cc-cvv-error" className="error" />
          </div>
        </div>
      </div>
      {!allPaymentMethodv2 && <div className="allowed-payments-logos">
        {processMada && <CardTypeSVG type="mada" key="card-mada" />}
