...
function App() {
  ...
  useEffect(() => {
    ...
    const orderId = params.get('reserveOrderId');
    const guestOrderToken = params.get('token');

    if (orderId && confirmationURI) {
      const secureOrderId = btoa(JSON.stringify({ reserveOrderId: orderId, token: guestOrderToken }));
