...
function OrderConfirmationSummary({
  ...
  const { placeholders, configs, auraAccountData, shouldEnableProductCardV2, isLoggedIn } = useContext(CartContext);
  ...
  const orderDetailsVersionTwo = configs?.['oredr-confirmation-order-details-v2'] === 'true';
  ...
  const formatPhoneNumber = (phoneNumber) => {
    // Parse the input phone number string
    const parsedNumber = parsePhoneNumberFromString(phoneNumber);

    if (parsedNumber) {
      // Get the country code and format the national part with " - " separators
      const countryCode = `+${parsedNumber.countryCallingCode}`;
      let nationalNumber = parsedNumber.formatNational().replace(/\s/g, ' - ');
      nationalNumber = nationalNumber.replace(/^0+/, ''); // Remove all leading 0s

      // Combine country code and national number with custom separators
      const customFormattedNumber = `${countryCode} - ${nationalNumber}`;
  ...
  const customerMobileNumber = formatPhoneNumber(
  ...
  const getPaymentInfo = () => {
    ...
    try {
      ...
      const methodsActualArray = methodsArray?.find(
        (item) => item.querySelectorAll('tr > td')?.[0]?.innerText
          === data?.payment?.method,
      );

      if (
        data?.payment?.method === AppConstants.PAYMENT_METHOD_TABBY
        || data?.payment?.method === AppConstants.PAYMENT_METHOD_CODE_TAMARA
      ) {
        const getPaymentTitle = () => {
          ...
          if (
            ...
            const paymentMethodTitleString = methodsActualArray?.querySelectorAll('tr > td')?.[1]?.innerHTML
              || '';
            paymentMethodTitle = paymentMethodTitleString;
          }

          return (
            <span dangerouslySetInnerHTML={{ __html: paymentMethodTitle }} />
          );
        };
        paymentMethodTitle = getPaymentTitle();
      } else {
        paymentMethodTitle = methodsActualArray?.querySelectorAll('tr > td')?.[1]?.innerText || '';
      }

      const hpsPayment = data?.payment?.additional_information?.find(
        (d) => d.indexOf('payment_method') !== -1,
      );
      const addditionalData = hpsPayment ? JSON.parse(hpsPayment) : null;
      if (
        addditionalData.payment_method !== data?.payment?.method
        && addditionalData.payment_method === AppConstants.hps_payment_method
      ) {
        const hpsPaymentDetail = methodsArray?.find(
          (item) => item.querySelectorAll('tr > td')?.[0]?.innerText
            === AppConstants.hps_payment_method,
        );
  ...
  const confirmationId = data?.extension_attributes?.payment_additional_info?.find(
    (item) => item.key === 'confirmation_id' || item.key === 'knet_transaction_id',
  );
  const payemtId = data?.extension_attributes?.payment_additional_info?.find(
    (item) => item.key === 'pun' || item.key === 'knet_payment_id',
  );
  const resultCode = data?.extension_attributes?.payment_additional_info?.find(
    (item) => item.key === 'status_message' || item.key === 'status',
  )
    ?? data?.extension_attributes?.payment_additional_info?.find(
      (item) => item.key === 'status',
    );
  const referenceId = data?.extension_attributes?.payment_additional_info?.find(
    (item) => item.key === 'reference_number',
  );
  const expiryTime = data?.extension_attributes?.payment_additional_info?.find(
    (item) => item.key === 'payment_expiry_time',
  );
  ...
  const getEgiftInfo = () => {
    ...
    data?.items?.forEach((d) => {
      if (
        d?.extension_attributes?.product_options?.[0]?.indexOf(
          'hps_giftcard_recipient_email',
        ) != -1
      ) {
        const productOptions = JSON.parse(
          d.extension_attributes.product_options[0],
        );
  ...
  const [paymentSummaryLabel, paymentSummaryValue] = content?.paymentSummary?.[0]?.querySelectorAll('tr td') || {};

  console.log(shouldEnableProductCardV2, "showBBWAuraBanner");

  return (
    <div className="order-confirmation-summary">
      {!orderDetailsVersionTwo && (
      ...
      <div className="order-confirmation-summary-main-content">
        <div className="heading">
          <span>
            {orderDetailsVersionTwo
              ? placeholders.orderDetailsTitle
              : placeholders.orderSummaryTitle}
          </span>
        </div>
        <div className="summary">
          {data?.payment?.method === AppConstants.PAYMENT_METHOD_CODE_FAWRY && (
            <>
              <div className="orderSummary-fawry-info-message">
                <span
                  className="message"
                  dangerouslySetInnerHTML={{
                    __html: paymentSummaryLabel?.innerHTML,
                  }}
                />
                <span
                  dangerouslySetInnerHTML={{
                    __html: paymentSummaryValue?.innerHTML,
                  }}
                />
              </div>
              <SummaryItem
                label={placeholders.fawryAmountDue}
                value={`${data.order_currency_code} ${data.base_grand_total}`}
              />
              <SummaryItem
                label={placeholders.fawryReferenceNumber}
                value={referenceId.value}
              />
              <SummaryItem
                label={placeholders.fawryCompletePaymentBy}
                value={expiryTime.value}
              />
            </>
          )}
          {!orderDetailsVersionTwo && emailId && (
            <SummaryItem
          ...
          {isOrderDetailExpanded && (
            <>
              ...
              {orderdataValues?.topupReceipientEmail && (
                <AccordionItem
      ...
      {shouldEnableProductCardV2 && isAuraEnabled ? (
        <>
          {!isLoggedIn && (auraAccountData.apc_link !== 2 || auraAccountData.apc_link !== 3) ? null :
            <AuraDetails orderItems={{
              auraEarnedPoints: data?.extension_attributes?.apc_accrued_points,
              auraRedeemedPoints: data?.extension_attributes?.apc_redeemed_points,
              auraSavedPoints: data?.extension_attributes?.hps_redeemed_amount,
            }}
            />}
        </>
      ) : (
        <div className="order-confirmation-summary-main-content aurapoints">
          ...
          {earnedAuraPoint > 0 && (
            <div className="download-text">
              <span
                dangerouslySetInnerHTML={{ __html: content.downloadText }}
              />
            </div>
          )}
