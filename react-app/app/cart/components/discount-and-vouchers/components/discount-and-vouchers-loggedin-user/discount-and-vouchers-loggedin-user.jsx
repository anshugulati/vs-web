...
function DiscountAndVouchersLoggedinUser() {
      ...
      if (selectedOfferItem) {
        try {
          const payload = !shouldRemove ? {
            ...
            offerType: selectedOfferItem.type ?? '',
          } : null;
  ...
  useEffect(() => {
    let foundVals = {};
    let eventLabelData = [];
    let actionLabel = 'Member Offers';

    if(selectedOffer){
      foundVals = allOffers?.find((val) => val?.code == selectedOffer);
      eventLabelData.push(foundVals?.description);
      
      handleDatalayerHelloMembersOffersSelection(foundVals?.promotion_type || actionLabel, eventLabelData);
    }
  }, [selectedOffer])
  ...
  useEffect(() => {
    let foundVals = {};
    let eventLabelData = [];
    let actionLabel = 'Bonus Vouchers';

    if(selectedVouchers.length > 0){
      selectedVouchers.map((selectedVal, i)=>{
        foundVals = allVouchers?.find((val) => val?.code == selectedVal);
        eventLabelData.push(foundVals?.description);
      })
  ...
  const handleDatalayerHelloMembersOffersSelection = (actionLabel, data) =>{
    window.dispatchEvent(new CustomEvent(
      'react:datalayerEvent',
      {
        detail: {
          type: 'helloMembersOffersSelection',
          payload: {
            eventAction: actionLabel || null,
            eventLabelData: data.join('|') || null
          },
        }
      }
    )); 
  ...
  const handleDatalayerHelloMembersOffersApply = (selectedItem = null) =>{
    let foundVals = {};
    let data = [];

    if(Array.isArray(selectedItem)){
      selectedItem.map((selectedVal, i)=>{
        foundVals = allVouchers?.find((val) => val?.code == selectedVal);
        data.push(foundVals?.description);
      })
    }
    else{
      foundVals = allOffers?.find((val) => val?.code == selectedOffer);
    ...
    window.dispatchEvent(new CustomEvent(
      ...
      {
        detail: {
          type: 'helloMembersOffersApply',
          payload: {
            eventAction: foundVals?.promotion_type || null,
            eventLabelData: data.join('|') || null
          },
        }
      }
